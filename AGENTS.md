...
Inventory defaults/sorting follow-up (September 14): opening Inventory without a `status` query defaults to `IN_STOCK`; explicit `status=ALL` and `status=SOLD` remain URL-backed. Sortable headers use database ordering before the existing 50-row range, persist `sort`/`direction` through filters and pagination, and default to Number ascending. Purchase Price remains Owner-only in both rendering and sort authorization. The stored Warehouse location stays unchanged; display helpers render `location_type=WAREHOUSE` as `Склад` in UA and `Warehouse` in EN across Inventory, Product Detail, location selectors, history, transfers, and transfer PDFs.
