...
Tasks 1 through 5A define the current implemented baseline. Task 5B may integrate calculated prices into inventory and checkout, but must first define precedence between an explicit `inventory_items.selling_price` and a calculated rule price. Returns, refunds, transfers, receipts, and analytics remain future work and require their own scoped tasks.
