...
Task 3A is implemented locally but not yet committed. It replaces the Inventory placeholder with an RLS-backed list, filters, exact barcode lookup, product details, and owner/manager manual add and edit workflows. The UI intentionally exposes no deletion, importer, sales workflow, or pricing formula. Lint, typecheck, 20 tests, and the production build pass. Live owner checks passed for create, details, duplicate-barcode handling, edit, `REMOVED` status, filtering, and exact barcode lookup. The hosted database contains one clearly labeled `TEST-3A-001` verification item left as `REMOVED`. Manager and salesperson acceptance still requires dedicated accounts.
