...
- Payload to `complete_sale(p_shop_id, p_items, p_notes)` contains only inventory_item_id and discount_percent per item. Extra fields, including sale_price, are rejected. Missing discount defaults to 0 in SQL. Notes are optional PostgreSQL text without an application character limit; 1–100 unique item UUIDs per sale.
