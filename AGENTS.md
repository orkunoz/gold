...
- Transfer PDF downloads are regenerated from immutable transfer snapshots through the Owner-authorized Node route. The generator reads the repository-owned Unicode font from `public/fonts`, embeds its glyph widths, and returns a private A4 PDF with a stable `transfer-NNNNNN.pdf` filename. PDF generation remains independent from transfer creation. The bulk transfer RPC locks the selection, creates snapshots, updates every inventory location, verifies the row result, and rolls the entire transaction back on failure.
