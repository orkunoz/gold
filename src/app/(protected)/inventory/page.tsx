...
export default async function InventoryPage({ searchParams }: { searchParams: SearchParams }) {
  ...
  const filters: InventoryFilterValues = {
    ...
    producer: parameter(params, "producer"),
    size: parameter(params, "size"),
    weight: parameter(params, "weight"),
  ...
  const inventoryPromise = getInventoryItems(filters, page, 25, sort, direction);
  ...
  return <section>
    ...
    <InventoryFilters key={currentQuery} filters={filters} role={employee.role} categories={options.categories} producers={options.producers} sizes={options.sizes} shops={options.shops}>
    ...
    <div className="mt-5 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
      ...
      <UrlPagination page={page} count={totalPages} label={t("inventory.pages")} />
    </div></InventoryFilters>
  </section>;
}
