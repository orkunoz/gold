...
export default async function InventoryItemPage({params,searchParams}:{params:Promise<{id:string}>;searchParams:Promise<{error?:string}>}){
 ...
 const details=[
  ...
  [t("fields.priceUah"),formatPrice(item.price,locale)],[t("fields.status"),t(`status.${item.status}`)],[t("fields.shop"),locationDisplayName(item.shops,locale)??t("common.unassigned")],
