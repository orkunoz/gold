...
export default async function InventoryItemPage({ params,searchParams }: { params: Promise<{ id: string }>;searchParams:Promise<{error?:string}> }) {
  ...
  const details = [
    ...
    [t("fields.status"), t(`status.${item.status}`)], [t("fields.shop"), locationDisplayName(item.shops,locale) ?? t("common.unassigned")], [t("fields.barcode"), displayValue(item.barcode)],
  ];
