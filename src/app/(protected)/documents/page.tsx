...
export default async function DocumentsPage({ searchParams }: { searchParams: Promise<{ type?: string }> }) {
  ...
  const weightUnit = locale === "ua" ? "г" : "g";
    ...
    <div className="zl-table-wrap mt-4">
      {type === "transfers" ? <table className="zl-table min-w-[1050px]">
        ...
        <tbody>{(rows as Awaited<ReturnType<typeof getTransfers>>).map(row => <tr key={row.id}>
          ...
          <td className="zl-table-number whitespace-nowrap">{row.total_weight} {weightUnit}</td><td className="zl-table-number whitespace-nowrap">{formatTablePrice(row.total_value, locale)}</td><td><a className={downloadClass} href={`/api/transfers/${row.id}/pdf`} download>{t("transfers.downloadPdf")}</a></td>
        </tr>)}</tbody>
        ...
        <tbody>{(rows as Awaited<ReturnType<typeof getAddedProductDocuments>>).map(row => <tr key={row.id}>
          ...
          <td className="zl-table-number">{row.product_count}</td><td className="zl-table-number whitespace-nowrap">{row.total_weight} {weightUnit}</td><td className="zl-table-number whitespace-nowrap">{formatTablePrice(row.total_value, locale)}</td><td><a className={downloadClass} href={`/api/added-products/${row.id}/pdf`} download>{t("addedProducts.downloadPdf")}</a></td>
        </tr>)}</tbody>
      </table>}
    </div>
  </section>;
