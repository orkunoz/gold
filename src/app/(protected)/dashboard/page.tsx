...
import { getDashboardReport, getInventoryLocationCounts, getSalespersonInventorySummary } from "@/lib/dashboard/queries";
...
export default async function DashboardPage({ searchParams }: { searchParams: SearchParams }) {
  ...
  const financialMetricClass = "mt-1.5 whitespace-nowrap text-lg font-semibold tracking-[-.025em] sm:text-xl";
  ...
  const inventory = employee.role === "salesperson" && employee.shop_id ? await getSalespersonInventorySummary(employee.shop_id) : report.inventory;
    ...
    <div className={`grid items-stretch gap-4 ${sections.inventory ? "xl:grid-cols-[minmax(0,1.85fr)_minmax(22rem,1fr)]" : ""}`}>
      <section className="zl-surface flex min-w-0 flex-col p-4" aria-labelledby="statistics-title">
        ...
        <dl className={`mt-3 grid grid-cols-2 gap-2 ${employee.role === "owner" && report.profit ? "lg:grid-cols-4" : "lg:grid-cols-3"}`}>
          <div className="rounded-xl bg-stone-50 p-3.5"><dt className="text-xs text-stone-500">{t("dashboard.revenue")}</dt><dd className={`${financialMetricClass} text-stone-950`}><MetricCountUp key={`revenue:${statisticsPresentation}`} value={report.kpis.revenue} locale={locale} /></dd></div>
          {employee.role === "owner" && report.profit ? <div className="rounded-xl bg-stone-50 p-3.5"><dt className="text-xs text-stone-500">{t("dashboard.netProfit")}</dt><dd className={`${financialMetricClass} ${netProfitTone(report.profit.net_profit)}`}><MetricCountUp key={`profit:${statisticsPresentation}`} value={report.profit.net_profit} locale={locale} /></dd>{report.profit.missing_purchase_cost_items ? <p className="mt-1 text-[11px] leading-4 text-stone-500">{t("dashboard.missingPurchaseCost",{count:report.profit.missing_purchase_cost_items})}</p> : null}</div> : null}
