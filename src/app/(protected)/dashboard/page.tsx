...
export default async function DashboardPage({ searchParams }: { searchParams: SearchParams }) {
  ...
  return <section>
    ...
    <div className="mt-6 grid gap-4 sm:grid-cols-3"><Metric label="Revenue" value={formatPrice(report.kpis.revenue)} /><Metric label="Items sold" value={String(report.kpis.items_sold)} /><Metric label="Gold weight sold" value={grams(report.kpis.gold_weight_sold)} /></div>
    {sections.inventory && report.inventory ? <><div className="mt-10"><h2 className="text-xl font-semibold">Current inventory</h2></div><div className="mt-4 grid gap-4 sm:grid-cols-3"><Metric label="In-stock items" value={String(report.inventory.in_stock_items)} /><Metric label="In-stock gold weight" value={grams(report.inventory.in_stock_weight)} /><Metric label="Inventory value" value={formatPrice(report.inventory.customer_value)} note={`${report.inventory.missing_price_items} item(s) missing price`} /></div></> : null}
    {sections.comparisons && report.sales_over_time ? <div className="mt-10 min-w-0 overflow-hidden rounded-xl border border-stone-200 bg-white p-5 shadow-sm"><h2 className="text-xl font-semibold">Sales over time</h2><div className="mt-5 min-w-0"><SalesTrend points={report.sales_over_time} /></div></div> : null}
    {sections.comparisons ? <div className="mt-10"><ReportTable title="Sales by category" headings={["Category","Items","Revenue","Weight"]} empty="No category sales in this period." rows={(report.categories ?? []).map((row) => [row.category,String(row.items_sold),formatPrice(row.revenue),grams(row.weight_sold)])} /></div> : null}
    {sections.shopPerformance && report.shops ? <div className="mt-10"><ReportTable title="Shop performance" headings={["Shop","Revenue","Items","Gold weight","In stock","Stock weight","Inventory value"]} empty="No active shops." rows={report.shops.map((row) => [row.shop,formatPrice(row.revenue),String(row.items_sold),grams(row.weight_sold),String(row.in_stock_items),grams(row.in_stock_weight),formatPrice(row.inventory_value)])} /></div> : null}
    <div className="mt-10"><h2 className="text-xl font-semibold">Recent sales</h2><div className="mt-4 overflow-hidden rounded-xl border border-stone-200 bg-white shadow-sm">{report.recent_sales.length ? <div className="overflow-x-auto"><table className="w-full min-w-[760px] text-left text-sm"><thead className="border-b border-stone-200 bg-stone-50 text-xs uppercase text-stone-500"><tr>{["Sale","Date/Time","Product Category",...(employee.role === "owner" ? ["Shop","Employee"] : []),"Items","Total",""] .map((heading) => <th key={heading} className="px-4 py-3">{heading}</th>)}</tr></thead><tbody className="divide-y divide-stone-100">{report.recent_sales.map((sale) => <tr key={sale.id}><td className="px-4 py-3 font-semibold">{sale.sale_number}</td><td className="px-4 py-3 whitespace-nowrap">{formatDateTime(sale.sold_at)}</td><td className="px-4 py-3">{sale.category_summary}</td>{employee.role === "owner" ? <><td className="px-4 py-3">{sale.shop}</td><td className="px-4 py-3">{sale.employee}</td></> : null}<td className="px-4 py-3">{sale.item_count}</td><td className="px-4 py-3 whitespace-nowrap">{formatPrice(sale.total_sale_price)}</td><td className="px-4 py-3"><Link href={`/sales/${sale.id}`} className="font-medium text-amber-900 hover:underline">View</Link></td></tr>)}</tbody></table></div> : <p className="p-10 text-center text-sm text-stone-500">No completed sales yet.</p>}</div></div>
  </section>;
}
