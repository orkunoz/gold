...
import { getSalesRegister, getSalesRegisterOptions, getSoldProductsRegister } from "@/lib/sales/queries";
import { inventoryOrdinal, inventoryResultSummary } from "@/lib/inventory/pagination";
import { parseSalesRegisterFilters, productSummary, salesPresetRange, salesRegisterHref } from "@/lib/sales/register";
...
export default async function SalesPage({ searchParams }: { searchParams: SearchParams }) {
  ...
  const period=typeof params.period==="string"?params.period:"";const preset=salesPresetRange(period);const fromDate=typeof params.from==="string"&&params.from||preset.from;const toDate=typeof params.to==="string"&&params.to||preset.to;
  const productFilters={shopId:filters.shopId,category:filters.category,producer:typeof params.producer==="string"&&params.producer||null,metal:typeof params.metal==="string"&&params.metal||null,employeeId:typeof params.employee==="string"&&params.employee||null,from:fromDate?`${fromDate}T00:00:00+03:00`:null,to:toDate?`${toDate}T00:00:00+03:00`:null,search:typeof params.search==="string"&&params.search||null,page:filters.page};
  const [employee, options, registerOptions, history,products] = await Promise.all([getCurrentEmployee(),getInventoryOptions(),getSalesRegisterOptions(),getSalesRegister(filters),getSoldProductsRegister(productFilters)]);
  ...
  const productPageHref=(nextPage:number)=>{const query=new URLSearchParams();Object.entries(params).forEach(([key,value])=>{if(typeof value==="string"&&key!=="page"&&value)query.set(key,value)});query.set("view","products");if(nextPage>1)query.set("page",String(nextPage));return `/sales?${query.toString()}`};
  ...
  return <section>
    ...
    {view==="products"?<>
    <form method="get" className="mt-5 grid gap-4 rounded-xl border border-stone-200 bg-stone-50 p-4 md:grid-cols-3"><input type="hidden" name="view" value="products"/>
      {employee.role==="owner"?<label className="text-sm font-medium">Shop<select name="shop" defaultValue={productFilters.shopId??""} className="mt-2 w-full rounded-lg border p-2.5"><option value="">All Shops</option>{registerOptions.shops.map(s=><option key={s.id} value={s.id}>{s.name}</option>)}</select></label>:null}
      <label className="text-sm font-medium">Category<select name="category" defaultValue={productFilters.category??""} className="mt-2 w-full rounded-lg border p-2.5"><option value="">All categories</option>{registerOptions.categories.map(c=><option key={c.id} value={c.name}>{c.name}</option>)}<option value="Uncategorized">Uncategorized</option></select></label>
      <label className="text-sm font-medium">Metal<select name="metal" defaultValue={productFilters.metal??""} className="mt-2 w-full rounded-lg border p-2.5"><option value="">All metals</option><option>Gold</option><option>Silver</option></select></label>
      <label className="text-sm font-medium">Producer<input name="producer" defaultValue={productFilters.producer??""} className="mt-2 w-full rounded-lg border p-2.5"/></label>
      <label className="text-sm font-medium">Salesperson<select name="employee" defaultValue={productFilters.employeeId??""} className="mt-2 w-full rounded-lg border p-2.5"><option value="">All staff</option>{registerOptions.employees.map(e=><option key={e.id} value={e.id}>{e.full_name??"Staff member"}</option>)}</select></label>
      <label className="text-sm font-medium">Search<input name="search" defaultValue={productFilters.search??""} placeholder="Article, barcode, producer, sale number" className="mt-2 w-full rounded-lg border p-2.5"/></label>
      <label className="text-sm font-medium">Period<select name="period" defaultValue={period} className="mt-2 w-full rounded-lg border p-2.5"><option value="">All dates / custom</option><option value="today">Today</option><option value="yesterday">Yesterday</option><option value="last7">Last 7 days</option><option value="month">This month</option></select></label>
      <label className="text-sm font-medium">From<input type="date" name="from" defaultValue={typeof params.from==="string"?params.from:""} className="mt-2 w-full rounded-lg border p-2.5"/></label><label className="text-sm font-medium">To (exclusive)<input type="date" name="to" defaultValue={typeof params.to==="string"?params.to:""} className="mt-2 w-full rounded-lg border p-2.5"/></label>
      <div className="flex items-end gap-2"><button className="rounded-lg bg-stone-900 px-4 py-2.5 text-sm font-medium text-white">Apply filters</button><Link href="/sales" className="rounded-lg border bg-white px-4 py-2.5 text-sm font-medium">Clear filters</Link></div>
    </form><p className="mt-5 font-medium">{inventoryResultSummary(products.page,products.pageSize,products.count,products.products.length)}</p>
