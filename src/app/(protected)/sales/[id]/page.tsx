...
export default async function SaleDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <section className="max-w-5xl">
    ...
    <div className="mt-8 overflow-x-auto rounded-xl border border-stone-200 bg-white"><table className="w-full min-w-[760px] text-left text-sm">
      <thead className="border-b border-stone-200 bg-stone-50 text-xs uppercase text-stone-500"><tr>{["Barcode", "Article", "Category", "Weight", "List snapshot", "Final price"].map((heading) => <th key={heading} className="px-4 py-3">{heading}</th>)}</tr></thead>
      <tbody className="divide-y divide-stone-100">{items.map((item) => <tr key={item.id}><td className="px-4 py-3 font-semibold">{item.inventory_items?.barcode ?? "—"}</td><td className="px-4 py-3">{displayValue(item.inventory_items?.article_number)}</td><td className="px-4 py-3">{item.inventory_items?.product_categories?.name ?? "—"}</td><td className="px-4 py-3">{item.inventory_items?.weight_grams === null || item.inventory_items?.weight_grams === undefined ? "—" : `${item.inventory_items.weight_grams} g`}</td><td className="px-4 py-3">{formatPrice(item.list_price)}</td><td className="px-4 py-3 font-semibold">{formatPrice(item.sale_price)}</td></tr>)}</tbody>
    </table></div>
  </section>;
}
