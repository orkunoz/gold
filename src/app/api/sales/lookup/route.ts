...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const {data:articleItems,error:articleError}=await supabase.from("inventory_items").select(selection).eq("article_number",code).eq("shop_id",shopId).order("created_at").limit(50);
    ...
    if(items.length===1)return NextResponse.json({item:items[0],matchedBy:"article"});
    return NextResponse.json({items,matchedBy:"article"});
  } catch {
