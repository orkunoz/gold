...
export async function GET(request: NextRequest) {
  ...
  if (!code || !shopId) return NextResponse.json({ error: t("sales.errors.lookupRequired") }, { status: 400 });
  if (employee.role !== "owner" && employee.shop_id !== shopId) return NextResponse.json({ error: t("sales.errors.shopForbidden") }, { status: 403 });
  ...
  try {
    ...
    return NextResponse.json({ error: t("sales.errors.pricingFailed") }, { status: 500 });
  }
}
