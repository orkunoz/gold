...
import { normalizeHeader } from "@/lib/inventory/import/headers";
import { IMPORT_FIELDS, type ColumnMapping, type ImportPreview, type ParsedSheet } from "@/lib/inventory/import/types";
import { useI18n } from "./i18n-provider";

type Result = { imported: number; skipped: number; ignoredRows: number; duplicates: number; failed: number; failures: { row: number; message: string }[] };
...
const fieldKeys: Partial<Record<typeof IMPORT_FIELDS[number], string>> = {
  ...
  weight_grams: "weight", price_per_gram: "pricePerGram", article_number: "article", barcode: "barcode",
};
const unsupportedSourceHeaders = new Set([
  "№ з/п", "№", "number", "row", "row number", "ціна(грн)", "ціна грн", "price", "total price",
  "дата реалізації", "date of sale", "sale date", "примітка", "примітки", "notatki", "notes", "note",
  "metal", "метал", "fineness", "проба", "discount", "знижка", "status", "статус",
  "shop", "store", "location", "магазин", "крамниця",
].map(normalizeHeader));
function showMappingSource(header:string,sourceIndex:number,mapping:ColumnMapping){
  return IMPORT_FIELDS.some(field=>mapping[field]===sourceIndex)||!unsupportedSourceHeaders.has(normalizeHeader(header));
}
  ...
  return <div>
    ...
    {!result ? <div className="mt-8 space-y-8">
      ...
      <section><h2 className="text-lg font-semibold">3. {t("import.mapping")}</h2>
        <div className="mt-4 grid gap-3 sm:grid-cols-2 xl:flex xl:flex-nowrap xl:items-start">
          {parsed.headers.map((header, sourceIndex) => ({header,sourceIndex})).filter(({header,sourceIndex})=>showMappingSource(header,sourceIndex,mapping)).map(({header,sourceIndex}) => { const selected = IMPORT_FIELDS.find((field) => mapping[field] === sourceIndex) ?? ""; const width=selected==="category"||selected==="producer"?"xl:flex-[1.4_1_0]":selected==="article_number"?"xl:flex-[1.15_1_0]":selected==="size"||selected==="weight_grams"?"xl:flex-[0.75_1_0]":"xl:flex-1"; return <label key={`${header}-${sourceIndex}`} className={`min-w-0 text-sm font-medium ${width}`}>{header}
      ...
      {preview ? <section className="border-t border-stone-200 pt-8"><h2 className="text-lg font-semibold">4. {t("import.preview")}</h2>
        ...
        <p className="mt-4 text-sm font-medium">{t("import.preImportSummary",{ready:preview.summary.ready,attention:preview.summary.warnings,blocked:preview.summary.errors,duplicateSummary:t("import.duplicateSummary",{count:preview.summary.duplicates}),ignoredSummary:t("import.ignoredSummary",{count:preview.summary.ignoredRows})})}</p>
        <div className="mt-5 max-h-96 overflow-auto rounded-lg border border-stone-200"><table className="w-full min-w-[1200px] text-left text-sm"><thead className="sticky top-0 bg-stone-50"><tr>{["import.sourceRow","import.result","fields.productCategory","fields.article","fields.producer","fields.size","fields.weight","fields.purchasePrice","fields.pricePerGram","import.price","fields.status","fields.shop","fields.barcode","import.issues"].map((heading,index)=><th key={heading} className={`px-3 py-2 ${index>=6&&index<=9?"text-right":""}`}>{t(heading)}</th>)}</tr></thead><tbody>{preview.rows.slice(0,200).map(row=><tr key={row.sourceRow} className={row.classification==="Error"?"bg-red-50":row.classification==="Warning"?"bg-amber-50":""}>{[row.sourceRow,t(`import.${row.classification.toLowerCase()}`),row.item?.category_name,row.item?.article_number,row.item?.producer,row.item?.size,row.item?.weight_grams,row.item?.purchase_price,row.item?.price_per_gram,row.item?.price,row.item?t(`status.${row.item.status}`):null,row.item?.shop_name,row.item?.barcode,[...row.errors,...row.warnings].join("; ")].map((value,index)=><td key={index} className={`px-3 py-2 ${index>=6&&index<=9?"text-right":""}`}>{value??"—"}</td>)}</tr>)}</tbody></table></div>
