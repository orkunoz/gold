  ...
  return <div>
    ...
    {!result ? <div className="mt-8 space-y-8">
      ...
      {preview ? <section className="border-t border-stone-200 pt-8"><h2 className="text-lg font-semibold">4. {t("import.preview")}</h2>
        ...
        <div className="mt-4 grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-6">{Object.entries(preview.summary).map(([label, value]) => <div key={label} className="rounded-lg bg-stone-100 p-3"><div className="text-2xl font-semibold">{value}</div><div className="text-xs text-stone-600">{t(`import.summary.${label}`)}</div></div>)}</div>
        <p className="mt-4 text-sm font-medium">{t("import.preImportSummary",{ready:preview.summary.ready,attention:preview.summary.warnings,blocked:preview.summary.errors,ignored:preview.summary.ignoredRows})}</p>
        {preview.ignoredSourceRows.length ? <p className="mt-2 text-sm text-stone-600">{t("import.ignoredSourceRows",{rows:preview.ignoredSourceRows.join(", ")})}</p> : null}
