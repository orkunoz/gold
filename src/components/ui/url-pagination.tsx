...
import {scrollToTopForPagination} from "./pagination-scroll";
...
export function UrlPagination({page,count,label}:{page:number;count:number;label:string}){
 ...
 const onPageChange=useCallback((next:number)=>{router.push(pageHref(pathname,searchParams.toString(),next),{scroll:false});scrollToTopForPagination()},[pathname,router,searchParams]);
