...
export function requestPageChange(current:number,total:number,next:number,onPageChange:(page:number)=>void){if(next>=1&&next<=total&&next!==current)onPageChange(next)}
...
export function Pagination({count,page,onPageChange,previousLabel,nextLabel,pageLabel,label,siblings=2,boundaries=1,className=""}:PaginationProps){
 ...
 const arrow="zl-pagination-arrow flex h-8 w-8 shrink-0 items-center justify-center rounded-[9px] text-stone-500 transition-colors enabled:hover:bg-stone-100 enabled:hover:text-stone-800 focus-visible:outline-2 focus-visible:outline-offset-1 focus-visible:outline-amber-700 disabled:opacity-35";
