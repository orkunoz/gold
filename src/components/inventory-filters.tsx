...
export function InventoryFilters({ filters, role, categories, shops, children }: {
  ...
  return <><form onSubmit={(event) => event.preventDefault()} className="mt-5 rounded-xl border border-stone-200 bg-white px-4 py-3 shadow-sm">
    <div className={`grid gap-3 sm:grid-cols-2 md:grid-cols-3 ${role === "owner" ? "lg:grid-cols-6" : "lg:grid-cols-5"}`}>
      <label className="min-w-0 text-xs font-medium">{t("fields.barcode")}<input name="barcode" value={draft.barcode} onChange={(event) => setDraft((current) => ({ ...current, barcode: event.target.value }))} className={control} /></label>
