...
describe("Task 15 focused UI",()=>{
 ...
 it("supports only current-page bulk selection and an Unassigned destination",()=>{const table=read("./inventory-bulk-table.tsx");expect(table).toContain("Select all on current page");expect(table).toContain("Clear selection");expect(table).toContain("Bulk Actions: Move to Shop");expect(table).toContain('canManage?<');expect(table).not.toMatch(/all pages|entire inventory/i);});
