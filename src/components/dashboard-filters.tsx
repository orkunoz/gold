...
export function DashboardFilters({ period: initialPeriod, start: initialStart, end: initialEnd, shopId: initialShopId, shops, isOwner }: {
  ...
  return <form onSubmit={submit} className="flex w-full flex-col gap-2 sm:w-auto sm:flex-row sm:flex-wrap sm:items-end" aria-busy={isPending}>
    ...
    {isOwner ? <label className="text-xs font-medium text-stone-600">{t("fields.shop")}<select name="shop" value={shopId} onChange={(event) => changeShop(event.target.value)} className={control}><option value="">{t("dashboard.allShops")}</option>{shops.map((shop) => <option key={shop.id} value={shop.id}>{locationDisplayName(shop,locale)}</option>)}</select></label> : null}
