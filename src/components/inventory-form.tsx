...
export function InventoryForm({action,categories,shops,item,categoryName,cancelHref}:Props){
 ...
 return <form action={formAction} className="space-y-8" aria-busy={pending}>{state.error?<div role="alert" className="rounded-lg border border-red-200 bg-red-50 p-4 text-sm text-red-800">{state.error}</div>:null}<div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
  ...
  <input type="hidden" name="owner_price" value={item?.owner_price??""}/><input type="hidden" name="selling_price" value={item?.selling_price??""}/><input type="hidden" name="discount" value={item?.discount??""}/><input type="hidden" name="received_at" value={item?.received_at?.slice(0,10)??""}/><input type="hidden" name="gold_fineness" value={item?.gold_fineness??""}/><input type="hidden" name="gold_color" value={item?.gold_color??""}/>
