...
.added-products-note .transfer-note__col--nr{width:3.5%}
.added-products-note .transfer-note__col--category{width:13%}
.added-products-note .transfer-note__col--article{width:7%}
.added-products-note .transfer-note__col--producer{width:11%}
.added-products-note .transfer-note__col--size{width:5%}
.added-products-note .transfer-note__col--weight{width:6%}
.added-products-note .transfer-note__col--price-per-gram{width:14%}
.added-products-note .transfer-note__col--price{width:13%}
.added-products-note .transfer-note__col--location{width:10.5%}
.added-products-note .transfer-note__col--barcode{width:17%}
...
@media print{.added-products-note.transfer-note--compact-table .transfer-note__signatures{margin-top:8mm;padding-bottom:0}}
`;
...
export function AddedProductsNote({document,items,labels,locale,logoSrc}:{document:AddedProductsDocument;items:AddedProductsItem[];labels:AddedProductsLabels;locale:"ua"|"en";logoSrc:string}){
 ...
 return <><style dangerouslySetInnerHTML={{__html:TRANSFER_NOTE_CSS+RECEIPT_CSS}}/><article className={`transfer-note added-products-note${compact?" transfer-note--compact-table":""}`} lang={locale==="ua"?"uk":"en"}>
  ...
  <div className="transfer-note__table-wrap"><table className="transfer-note__table"><colgroup>{ADDED_PRODUCTS_COLUMNS.map(column=><col key={column} className={`transfer-note__col--${column}`}/>)}</colgroup><thead><tr>{labels.headings.map((heading,index)=><th key={heading} className={`transfer-note__cell--${ADDED_PRODUCTS_COLUMNS[index]}`}>{heading}</th>)}</tr></thead><tbody>{items.map(x=><tr key={x.id??x.line_number}>{[x.line_number,x.category_name,x.article_number,x.producer,x.size,number.format(x.weight_grams),priceNumber.format(x.price_per_gram),priceNumber.format(x.price),labels.location(x.location_name),x.barcode].map((value,index)=><td key={index} className={`transfer-note__cell--${ADDED_PRODUCTS_COLUMNS[index]}`}>{display(value)}</td>)}</tr>)}</tbody></table></div>
