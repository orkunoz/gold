...
export function AddedProductsNote({document,items,labels,locale,logoSrc}:{document:AddedProductsDocument;items:AddedProductsItem[];labels:AddedProductsLabels;locale:"ua"|"en";logoSrc:string}){
 const language=locale==="ua"?"uk-UA":"en-US",number=new Intl.NumberFormat(language,{maximumFractionDigits:3}),priceNumber=new Intl.NumberFormat(language,{minimumFractionDigits:2,maximumFractionDigits:2}),money=new Intl.NumberFormat(language,{style:"currency",currency:"UAH"}),weightUnit=documentWeightUnit(locale),totalValue=locale==="ua"?priceNumber.format(document.total_value):money.format(document.total_value);
 ...
 return <><style dangerouslySetInnerHTML={{__html:TRANSFER_NOTE_CSS+RECEIPT_CSS}}/><article className={`transfer-note added-products-note${locale==="ua"?" transfer-note--ua-table":""}${compact?" transfer-note--compact-table":""}`} lang={locale==="ua"?"uk":"en"}>
  ...
  <div className="transfer-note__table-wrap"><table className="transfer-note__table"><colgroup>{ADDED_PRODUCTS_COLUMNS.map(column=><col key={column} className={`transfer-note__col--${column}`}/>)}</colgroup><thead><tr>{labels.headings.map((heading,index)=><th key={heading} className={`transfer-note__cell--${ADDED_PRODUCTS_COLUMNS[index]}`}><DocumentTableHeading heading={heading}/></th>)}</tr></thead><tbody>{items.map(x=><tr key={x.id??x.line_number}>{[x.line_number,x.category_name,x.article_number,x.producer,x.size,number.format(x.weight_grams),priceNumber.format(x.price_per_gram),priceNumber.format(x.price),labels.location(x.location_name),x.barcode].map((value,index)=><td key={index} className={`transfer-note__cell--${ADDED_PRODUCTS_COLUMNS[index]}${index===9&&typeof value==="string"&&value.length>13?" transfer-note__cell--long-id":""}`}>{display(value)}</td>)}</tr>)}</tbody></table></div>
  <section className="transfer-note__summary"><div><span>{labels.products}</span><strong>{document.product_count}</strong></div><div><span>{labels.totalWeight}</span><strong>{number.format(document.total_weight)} {weightUnit}</strong></div><div><span>{labels.totalValue}</span><strong>{totalValue}</strong></div></section>
