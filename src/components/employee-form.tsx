...
export function EmployeeForm({action,shops,employee,invite=false}:{action:Action;shops:Shop[];employee?:Employee;invite?:boolean}) {
  ...
  return <form action={formAction} onSubmit={confirmSensitive} className="max-w-2xl space-y-5">
    ...
    {invite?<><label className="block text-sm font-medium">Username<input name="username" required minLength={3} maxLength={32} pattern="[a-z0-9][a-z0-9_-]{2,31}" autoCapitalize="none" spellCheck={false} className="mt-2 w-full rounded-lg border border-stone-300 px-3 py-2.5"/></label><label className="block text-sm font-medium">Password<input name="password" type="password" required minLength={6} maxLength={72} autoComplete="new-password" className="mt-2 w-full rounded-lg border border-stone-300 px-3 py-2.5"/></label><label className="block text-sm font-medium">Confirm password<input name="password_confirmation" type="password" required minLength={6} maxLength={72} autoComplete="new-password" className="mt-2 w-full rounded-lg border border-stone-300 px-3 py-2.5"/></label></>:<p className="text-sm"><span className="font-medium">Username:</span> {employee?.username||"Legacy account"}</p>}
    {!invite?<label className="block text-sm font-medium">Display name<input name="full_name" required maxLength={200} defaultValue={employee?.full_name??""} className="mt-2 w-full rounded-lg border border-stone-300 px-3 py-2.5"/></label>:null}
    {invite ? <div className="block text-sm font-medium">Role<p className="mt-2 rounded-lg border border-stone-200 bg-stone-100 px-3 py-2.5 capitalize">salesperson</p></div> : <label className="block text-sm font-medium">Role<select name="role" defaultValue={employee?.role??"salesperson"} className="mt-2 w-full rounded-lg border border-stone-300 bg-white px-3 py-2.5">{ADMIN_ROLES.map(role=><option key={role} value={role}>{role}</option>)}</select></label>}
    <label className="block text-sm font-medium">Assigned shop<select name="shop_id" required={invite} defaultValue={employee?.shop_id??""} className="mt-2 w-full rounded-lg border border-stone-300 bg-white px-3 py-2.5"><option value="">{invite?"Select shop":"No shop (Owner only)"}</option>{shops.map(shop=><option key={shop.id} value={shop.id}>{shop.name}</option>)}</select></label>
    ...
    <div className="flex flex-col gap-3 sm:flex-row"><button disabled={pending} className="rounded-lg bg-stone-900 px-5 py-2.5 text-sm font-medium text-white">{pending?"Saving…":invite?"Create account":"Save account"}</button><Link href="/admin/employees" className="px-5 py-2.5 text-center text-sm">Cancel</Link></div>
  </form>;
}
