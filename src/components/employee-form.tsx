...
export function EmployeeForm({action,shops,employee,invite=false}:{action:Action;shops:Shop[];employee?:Employee;invite?:boolean}) {
  ...
  return <form action={formAction} onSubmit={confirmSensitive} className="max-w-2xl space-y-5">
    ...
    <div className="flex gap-3"><button disabled={pending} className="rounded-lg bg-stone-900 px-5 py-2.5 text-sm font-medium text-white">{pending?"Saving…":invite?"Create account":"Save employee"}</button><Link href="/admin/employees" className="px-5 py-2.5 text-sm">Cancel</Link></div>
  </form>;
}
