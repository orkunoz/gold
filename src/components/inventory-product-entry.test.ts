...
const valid={shop_id:"warehouse",category_name:"Каблучка",article_number:"A-1",producer:"NewGold",size:"17",weight_grams:"2.4",purchase_price:"1000",price_per_gram:"1500",barcode:"001234"};
describe("manual product entry",()=>{
 it.each(["category_name","article_number","producer","weight_grams","purchase_price","price_per_gram"] as const)("rejects missing %s",field=>expect(draftValidation({...valid,[field]:""})).toHaveProperty(field,"required"));
 it.each(["category_name","article_number","producer"] as const)("rejects whitespace-only %s",field=>expect(draftValidation({...valid,[field]:"   "})).toHaveProperty(field,"required"));
 it.each([["weight_grams","0"],["weight_grams","nope"],["purchase_price","-1"],["purchase_price","nope"],["price_per_gram","0"],["price_per_gram","nope"]] as const)("rejects invalid %s",(field,value)=>expect(draftValidation({...valid,[field]:value})).toHaveProperty(field,"invalid"));
 ...
 it("uses the final proportional data row with Scan and Add below",()=>{const source=read("./inventory-draft-basket.tsx");expect(source).toContain("xl:grid-cols-[minmax(0,1.25fr)_minmax(0,1fr)_minmax(0,1fr)_minmax(0,0.75fr)_minmax(0,0.625fr)_minmax(0,0.75fr)_minmax(0,0.75fr)_minmax(116px,0.85fr)_minmax(0,1fr)_minmax(0,1fr)]");expect(source).toContain('className="mt-4 flex justify-end gap-2"');expect(source).toContain("readOnly tabIndex={-1}");expect(source).toContain('className={`${input} overflow-hidden whitespace-nowrap bg-stone-100 tabular-nums`}');expect(source).toContain('className="h-10 whitespace-nowrap rounded-lg')});
 it("widens only the basket Product Category column by reducing Size and Weight",()=>{const source=read("./inventory-draft-basket.tsx");expect(source).toContain('<col className="w-[16%]"/>');expect(source.match(/<col className="w-\[6\.25%\]"\/>/g)).toHaveLength(2);expect(source.match(/<col className="w-\[9\.5%\]"\/>/g)).toHaveLength(7)});
...
describe("required-field UX",()=>{it("uses localized field errors without a global banner or raw key",()=>{const basket=read("./inventory-draft-basket.tsx"),en=read("../../locales/en.json"),ua=read("../../locales/ua.json");expect(basket).not.toContain("requiredFields");expect(en).toContain('"required": "Required"');expect(ua).toContain('"required": "Обов\'язково"');expect(basket).toContain('t(`errors.${fieldErrors[f]}`)');expect(basket).not.toContain("validation.required")})});
