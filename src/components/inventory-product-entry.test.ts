...
describe("manual product entry",()=>{
 ...
 it("keeps every Add Product control in one proportional desktop row",()=>{const source=read("./inventory-draft-basket.tsx");expect(source).toContain("xl:grid-cols-[minmax(0,1fr)_minmax(0,1fr)_minmax(0,1fr)_minmax(0,0.5fr)_minmax(0,0.5fr)_minmax(0,1fr)_minmax(0,1fr)_minmax(0,0.5fr)_minmax(0,1fr)_auto_minmax(0,1fr)_auto]");expect(source).not.toContain("lg:grid-cols-[1.2fr_0.8fr_1fr]");expect(source).toContain('className="h-10 whitespace-nowrap rounded-lg')});
