...
describe("manual product entry",()=>{
 ...
 it("keeps product controls in one wider desktop row and the Add button below",()=>{const source=read("./inventory-draft-basket.tsx");expect(source).toContain("xl:grid-cols-[minmax(0,1fr)_minmax(0,1fr)_minmax(0,1fr)_minmax(0,0.5fr)_minmax(0,0.5fr)_minmax(0,1fr)_minmax(0,1fr)_minmax(116px,0.8fr)_minmax(0,1fr)_auto_minmax(0,1fr)]");expect(source).toContain('className="mt-4 flex justify-end"');expect(source).toContain("overflow-hidden whitespace-nowrap");expect(source).toContain("tabular-nums");expect(source).toContain('className="h-10 whitespace-nowrap rounded-lg')});
