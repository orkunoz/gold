...
describe("Excel header mapping", () => {
  ...
  it("suggests Ukrainian and English aliases", () => {
    expect(suggestColumnMapping(["Штрихкод", "Артикул", "Виріб", "Метал", "Виробник", "Вага", "Розмір", "Ціна", "Знижка", "Примітка", "Статус"]))
      .toMatchObject({ barcode: 0, article_number: 1, category: 2, producer: 4, weight_grams: 5, size: 6, discount: 8, notes: 9, status: 10 });
  ...
  it("maps both real shop workbook formats and ignores total UAH price",()=>{
    expect(suggestColumnMapping(["Виріб","Метал","Проба","Розмір","Вага","Ціна-грам","Артикул","Ціна(грн)","Примітка"])).toEqual({category:0,fineness:2,size:3,weight_grams:4,price_per_gram:5,article_number:6,notes:8});
