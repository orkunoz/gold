...
import {createEmployeeAccount,deleteEmployeeAccount,resetEmployeePassword} from "./actions";
...
describe("Owner account actions",()=>{
 ...
 it("authorizes in PostgreSQL before permanently deleting the Auth identity",async()=>{mocks.rpc.mockResolvedValue({data:"auth-2",error:null});mocks.deleteUser.mockResolvedValue({error:null});await expect(deleteEmployeeAccount("employee-2")).rejects.toThrow("REDIRECT:/admin/employees");expect(mocks.rpc).toHaveBeenCalledWith("admin_prepare_account_deletion",{p_employee_id:"employee-2"});expect(mocks.deleteUser).toHaveBeenCalledWith("auth-2");});
});
