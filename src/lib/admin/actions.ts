...
export async function updateShop(id:string,_state:AdminState,form:FormData):Promise<AdminState>{await requireOwner();const name=value(form,"name"),code=normalizeShopCode(value(form,"code"));const db=await createClient();const{error}=await db.rpc("admin_update_location",{p_shop_id:id,p_name:name,p_code:code,p_address:value(form,"address"),p_location_type:value(form,"location_type")||"SHOP"});if(error)return{error:error.code==="23505"?"Location code/type already exists.":"Unable to update location."};revalidatePath("/admin");redirect("/admin/shops");}
