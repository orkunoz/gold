...
export function employeeAssignmentValid(role:EmployeeRole,shopId:string,isActive=true){void role;void isActive;return !shopId||Boolean(shopId);}
export function friendlyAdminError(message:string){
  ...
  if(normalized.includes("last active owner"))return "The last active Owner cannot be deleted, deactivated, or assigned another role.";
