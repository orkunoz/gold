...
`inventory_items.price` is database-calculated as rounded `weight_grams × price_per_gram`; it is null when either input is missing. `discount` preserves informational XLSX source data and never changes checkout pricing. These remain separate from `owner_price`, manual `selling_price`, and pricing-rule effective price. Barcode is optional; multiple null barcodes are allowed, while every non-null barcode remains globally unique and scanner-compatible.
