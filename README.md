...
Checkout Discount % is per sale item, accepts 0–100 (including decimals), recalculates the editable final price, and is validated again by the atomic `complete_sale` RPC. `sale_items` permanently snapshots list price, discount percentage, final price, and product metadata. The Sales Register defaults to one-row-per-sold-product Products view and retains the one-row-per-transaction Sales view; product filters and pagination are database-side and shop-authorized.
