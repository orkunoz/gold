...
begin
  ...
  insert into public.sale_items(sale_id,inventory_item_id,sale_price,category_name) values
    (v_sale_a,v_item_a,100,'Task 9 Ring'),(v_sale_a,v_item_b,100,'Task 9 Bracelet'),(v_sale_b,v_item_c,100,'Task 9 Ring'),(v_sale_c,v_item_d,100,'Task 9 Bracelet');
  ...
  v_report:=public.get_sales_register(null,'Task 9 Ring',1,25);
  if (v_report->>'count')::int<>2 then raise exception 'owner category filter failed'; end if;
  v_report:=public.get_sales_register(v_shop_a,'Task 9 Bracelet',1,25);
  ...
  perform public.get_sales_register(null,'Task 9 Bracelet',1,25);
